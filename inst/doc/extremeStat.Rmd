...
you can use `distLquantile`, which internally calls `distLfit`, in the following manner 
...
printL(dlq)
```
dlf objects can be plotted with `plotLfit`, `plotLweights` and `plotLquantile`:
...
to determine **empirical percentiles** (order based statistic, keyword plotting positions).  
...
weighted by their goodness of fit (RMSE ecdf / cdf) in three default and a custom weighting schemes:  
...
They are aggregated by [rmse](https://www.rdocumentation.org/packages/berryFunctions/topics/rmse?)
...
In `distLquantile`, you can set the threshold manually, or (better) as a truncate percentage (`truncate` = **` t `** above) reflecting the proportion of data discarded:
