...
*This is the vignette for the R package `extremeStat`, available at [github.com/brry](https://github.com/brry/extremeStat).  (Vignette [Rmd source](https://raw.githubusercontent.com/brry/extremeStat/master/vignettes/quantileEstimation.Rmd))*
...
library(extremeStat)
```
extremeStat has 28 dependencies, because of the GPD comparison across the packages: `evd, evir, extRemes, fExtremes, ismev, lmomco, Renext`.
...
The function `distLfit` fits 17 of the distribution types avalable in the R package `lmomco` (there are more, but some of these require quite a bit of computation time and are prone to not be able to be fitted to this type of data distribution anyways).  
...
```{r teffplot, echo=-(1:4), fig.height=3.5, fig.width=5.5}
...
The cool thing about parametrical quantiles is that they don't systematically underestimate the actual quantile in small samples. 
...
Once you have quantiles, you can easily compute extremes for given return periods.  
A value x in a time series has a certain expected frequency to occur or be exceeded.

$$ RP = \frac{1}{exceedance ~  probability ~~ (=Pe)}     ~~   Pe = 1-quantile(x) $$
